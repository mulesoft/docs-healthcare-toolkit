...
HAPI TestPanel is an HL7 receiver, transmitter and message editor. It is free software distributed under the http://hl7api.sourceforge.net/license.html[MPL/GPL] licenses, and runs on Mac OSX, Linux and Windows.
...
After the HL7 inbound endpoint, an xref:3.7@mule-runtime::all-flow-control-reference.adoc[All Flow Control] sends the HL7 message to two xref:3.7@mule-runtime::cache-scope.adoc[Cache Scopes] for parallel processing.
...
In the `To XML and file` scope, the first building block, an xref:hl7-encoding-transformer.adoc[HL7 Encoding Transformer] transforms the original HL7 message from ER7 to XML, and a xref:3.7@mule-runtime::file-connector.adoc[File Endpoint] writes the output to a file (in this example, `/tmp/hl7-2test`).
